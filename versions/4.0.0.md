...
OAS Overlay documents are identified using the required field ['openapi-document-type'](#oasDocumentType) with a value that must match the string `"OAS Overlay"`.
...
This is the root document object of the [OpenAPI Overlay document](#oasOverlayObject).
...
<a name="oasDocumentType"></a>openapi-document-type | `string` | **REQUIRED**. This string MUST be the [`OAS Overlay`] value identifying the overlay document
...
<a name="oasOverlayDescription"></a>extends | string | Description of the overlay usage
...
## <a name="mergingAlgorithm></a>Appendix B: Mergint algorithm
